...
export default function CharactersSection() {
  ...
  if (!filtered.length) {
    return (
      <section
        className="relative py-16 px-4 text-center text-amber-50 bg-center bg-cover"
        style={{ backgroundImage: `url(${charactersBg})` }}
        id="characters"
      >
        ...
        <p className="font-cormorant italic mt-2">Our legends are still being written. Check back later!</p>
      </section>
    );
  }

  // --------------------------------------------
  // Admin add character mutation
  // --------------------------------------------
  ...
  return (
    <section
      className="relative py-16 px-4 bg-cover bg-center text-amber-50 overflow-hidden"
      style={{ backgroundImage: `url(${charactersBg})` }}
      id="characters"
    >
      ...
      <div className="relative container mx-auto max-w-6xl space-y-12">
        ...
        {isAdmin && (
          <div className="text-right">
            {!showAdd ? (
              <Button onClick={() => setShowAdd(true)} className="bg-amber-700 hover:bg-amber-800 font-cinzel">
                ➕ Add Character
              </Button>
            ) : (
